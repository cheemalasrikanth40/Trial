...
COPY --from=php /usr/bin/php /usr/bin/redis-server /usr/bin/redis-sentinel /usr/bin/redis-cli /usr/bin/redis-check-rdb /usr/bin/redis-check-aof /usr/bin/redis-benchmark /usr/bin/
...
#ELASTIC
axel https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-7.6.2-linux-x86_64.tar.gz && tar -xf  elasticsearch-7.6.2-linux-x86_64.tar.gz -C /usr/share/ \
&& echo -e "export ES_JAVA_HOME=/usr/lib/jvm/java-8-openjdk\nexport JAVA_HOME=/usr/lib/jvm/java-8-openjdk" >> /etc/profile \
&& mv /usr/share/elasticsearch-7.6.2* /usr/share/elasticsearch \
